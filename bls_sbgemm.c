/*
   ...
   What this file contains (minimal set):
   - A BF16 (uint16) 16x4 microkernel that accumulates into float32.
   - float32 -> BF16 packing routines that produce the microkernel’s expected
     packed-panel layout.
   - A small blocked driver (`bls_sgemm_via_bf16`) that:
       * packs float32 A/B to BF16, and
       * calls the microkernel over the full m x n result.

   This is intentionally sandbox-local code. It does NOT add a new BLIS datatype.
*/
...
// Tunables (defaults from the student project).
#ifndef BLS_BF16_MC
...
static void bls_microkernel_sbgemm_16x4
     (
       int             k,
       const uint16_t* a_packed,   // (k x MR)
       const uint16_t* b_packed,   // (k x NR)
       const float*    alpha,
       const float*    beta,
     ...
     )
{
	...
	// Initialize accumulators from C (or zeros if beta==0).
	if ( beta_s == 0.0f )
	{
	...
	else
	{
		for ( int j = 0; j < BLS_BF16_NR; ++j )
		{
			...
			// This microkernel assumes rs_c == 1 for the fast path where possible.
			// We still support general strides by gathering into a temp.
			if ( rs_c == 1 )
			{
			...
			{
				float tmp0[8], tmp1[8];
				for ( int i = 0; i < 8; ++i ) tmp0[i] = c_col[(0+i)*rs_c];
				for ( int i = 0; i < 8; ++i ) tmp1[i] = c_col[(8+i)*rs_c];
	...
	while ( p + 1 < k )
	{
		// A at p, p+1 (each provides MR=16 bf16s).
	...
	for ( int j = 0; j < BLS_BF16_NR; ++j )
	{
		...
		else
		{
			...
			for ( int i = 0; i < 8; ++i ) c_col[(0+i)*rs_c] = tmp0[i];
			for ( int i = 0; i < 8; ++i ) c_col[(8+i)*rs_c] = tmp1[i];
		}
	}
}
...
// ---- Sandbox hook: replace sgemm with BF16 microkernel -----------------------
     ...
     )
{
	...
	{
	// Fallback: scalar float32 GEMM (exact). This is just for robustness when
	// allocations fail; performance is not a goal here.
		...
		for ( dim_t ii = 0; ii < m; ++ii )
		{
			...
			{
				const float av = ( transa == BLIS_NO_TRANSPOSE ? a[ii*rs_a + pp*cs_a]
				                                              : a[pp*rs_a + ii*cs_a] );
				const float bv = ( transb == BLIS_NO_TRANSPOSE ? b[pp*rs_b + jj*cs_b]
				                                              : b[jj*rs_b + pp*cs_b] );
				acc += alpha_s * av * bv;
			}
	...
	for ( dim_t jc = 0; jc < n; jc += NC )
	{
		...
		for ( dim_t pc = 0; pc < k; pc += KC )
		{
			...
			// Pack B panels for this (kc x nc) block (float32 -> BF16).
			dim_t bidx = 0;
			...
			for ( dim_t ic = 0; ic < m; ic += MC )
			{
				...
				// Pack A panels for this (mc x kc) block (float32 -> BF16).
				dim_t aidx = 0;
				...
				// Compute microtiles via the BF16 microkernel.
				for ( dim_t jr = 0; jr < nc; jr += BLS_BF16_NR )
				{
					const dim_t nr = ( nc - jr < BLS_BF16_NR ? nc - jr : BLS_BF16_NR );
					...
					for ( dim_t ir = 0; ir < mc; ir += BLS_BF16_MR )
					{
						...
						else
						{
							...
							for ( dim_t jj = 0; jj < nr; ++jj )
							for ( dim_t ii = 0; ii < mr; ++ii )
								tmp[ii + jj*BLS_BF16_MR] = ( beta_eff == 0.0f ? 0.0f : beta_eff * c_tile[ii*rs_c + jj*cs_c] );
